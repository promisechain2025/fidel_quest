...
import { mintFamilyCode, mintRandomFamilyCode, isValidFamilyCode, ALPHABET } from '../familyCode.js'
// The anti-drift tripwire: the APP's own validator must accept our codes.
import { isValidFamilyCode as appIsValid } from '../../src/platform/familyPack.js'
...
test('FAM codes: mint matches the app algorithm exactly', () => {
  assert.equal(mintFamilyCode('ABCD'), 'FAMABCD' + ALPHABET[(0 * 3 + 1 * 4 + 2 * 5 + 3 * 6) % 31])
  for (let i = 0; i < 200; i++) {
    const code = mintRandomFamilyCode()
    assert.ok(isValidFamilyCode(code), `api validator rejects ${code}`)
    assert.ok(appIsValid(code), `APP validator rejects ${code} - algorithms drifted`)
  }
